...
/// Renvoie le temps depuis lequel le bot est en ligne
#[command(prefix_command, category = "Misc")]
pub async fn uptime(ctx: Context<'_>) -> Result<()> {
    let uptime_seconds = Utc::now().timestamp() - STARTING_TIME.timestamp();
    ctx.say(format!("Je suis actif sans interruption depuis {}s (en gros {} jours)", uptime_seconds, uptime_seconds / 86400))
        .await?;
    Ok(())
}
